      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 md:px-8 lg:px-16">
          ...
          {steps.map((step, index) => (
            ...
            >
              ...
              >
                ...
                <div className="relative bg-zinc-900/80 backdrop-blur-xl rounded-2xl md:rounded-3xl p-5 md:p-6 lg:p-8 border border-white/10 group-hover:border-white/30 transition-all duration-500">
                  <div className="flex items-center gap-3 md:gap-4 mb-5 md:mb-6">
                    <motion.div
                      className={`w-12 h-12 md:w-14 md:h-14 lg:w-16 lg:h-16 rounded-xl md:rounded-2xl bg-gradient-to-br ${step.gradient} flex items-center justify-center`}
                      whileHover={{ rotate: 360, scale: 1.1 }}
                      transition={{ duration: 0.6 }}
                    >
                      <step.icon className="w-6 h-6 md:w-7 md:h-7 lg:w-8 lg:h-8 text-white" />
                    </motion.div>
                    <span className={`text-4xl md:text-5xl lg:text-6xl font-montserrat font-black text-transparent bg-clip-text bg-gradient-to-r ${step.gradient} opacity-30`}>
                      {step.number}
                    </span>
                  </div>

                  <h3 className="text-lg md:text-xl font-montserrat font-bold text-white mb-2 md:mb-3">
                    {step.title}
                  </h3>
        ...
        >
          <motion.a
            ...
            whileTap={{ scale: 0.95 }}
            className="inline-flex items-center gap-2 md:gap-3 px-6 md:px-10 py-3 md:py-5 bg-gradient-to-r from-emerald-500 via-teal-500 to-cyan-500 text-white rounded-xl md:rounded-2xl font-montserrat font-bold text-sm md:text-base lg:text-lg shadow-2xl"
          >
