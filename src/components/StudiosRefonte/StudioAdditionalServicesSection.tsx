...
import { Star, ArrowRight, Check, Film, Settings, Users, ShoppingCart, Loader2 } from 'lucide-react';
...
export default function StudioAdditionalServicesSection() {
  ...
  const { products, loading: productsLoading } = useShopifyCollection('services-additionnels');
  ...
  const getCategoryFromTags = (tags: string[]): 'post-production' | 'equipement' | 'services' => {
    if (tags.some(tag => tag.toLowerCase().includes('post') || tag.toLowerCase().includes('montage') || tag.toLowerCase().includes('color'))) {
      return 'post-production';
    }
    if (tags.some(tag => tag.toLowerCase().includes('équipement') || tag.toLowerCase().includes('materiel'))) {
      return 'equipement';
    }
    return 'services';
  };

  const getGradientFromCategory = (category: 'post-production' | 'equipement' | 'services'): string => {
    switch (category) {
  ...
  const handleAddToCart = async (productId: string, productTitle: string, price: number, image?: string) => {
      ...
      <section className="relative py-16 md:py-24 lg:py-32">
        <div className="relative z-10 max-w-7xl mx-auto px-4 flex items-center justify-center">
          <div className="flex items-center gap-3 text-white">
            ...
            <span className="text-lg font-inter">Chargement des services...</span>
          </div>
        </div>
      </section>
    );
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 md:px-8 lg:px-16">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8 mb-10 md:mb-12 lg:mb-16">
          {filteredProducts.map((product, index) => {
            ...
            const category = getCategoryFromTags(product.tags);
            const gradient = getGradientFromCategory(category);
            const features = extractFeatures(product.description);
            const isPopular = product.tags.some(tag => tag.toLowerCase() === 'populaire');
            ...
            return (
              ...
              >
                ...
                >
                  <motion.div
                    className={`absolute -inset-1 bg-gradient-to-r ${gradient} rounded-3xl opacity-0 group-hover:opacity-30 blur-xl transition-all duration-500`}
                  />
                  ...
                  <div className="relative bg-zinc-900/80 backdrop-blur-xl rounded-2xl md:rounded-3xl p-5 md:p-6 lg:p-8 border border-white/10 group-hover:border-white/30 transition-all duration-500 h-full flex flex-col">
                    <div className="flex items-start justify-between mb-4 md:mb-5 lg:mb-6">
                      <motion.div
                        className={`w-12 h-12 md:w-14 md:h-14 lg:w-16 lg:h-16 rounded-xl md:rounded-2xl bg-gradient-to-br ${gradient} flex items-center justify-center overflow-hidden`}
                      ...
                      >
                        {image ? (
                          ...
                          <Settings className="w-6 h-6 md:w-7 md:h-7 lg:w-8 lg:h-8 text-white" />
                        )}
                      </motion.div>
                    </div>
                    ...
                    <p className="text-white/70 font-inter text-sm md:text-base leading-relaxed mb-3 md:mb-4 line-clamp-2">
                      {product.description.split('\n')[0] || 'Service professionnel pour votre production'}
                    </p>

                    <div className={`inline-flex items-baseline gap-1.5 md:gap-2 mb-4 md:mb-5 lg:mb-6 px-3 md:px-4 py-1.5 md:py-2 bg-gradient-to-r ${gradient} bg-opacity-10 border border-white/20 rounded-lg md:rounded-xl self-start`}>
                    ...
                    <div className="space-y-2 md:space-y-3 mb-5 md:mb-6 flex-1">
                      {features.map((feature, idx) => (
                        ...
                        >
                          <div className={`flex-shrink-0 w-4 h-4 md:w-5 md:h-5 rounded-full bg-gradient-to-br ${gradient} flex items-center justify-center mt-0.5`}>
                    ...
                    <motion.button
                      onClick={() => handleAddToCart(product.id, product.title, price, image)}
                      disabled={isAdding || cartLoading}
                      whileHover={{ scale: isAdding ? 1 : 1.02 }}
                      whileTap={{ scale: isAdding ? 1 : 0.98 }}
                      className={`w-full py-3 md:py-4 bg-gradient-to-r ${gradient} text-white rounded-lg md:rounded-xl font-inter font-medium text-sm md:text-base shadow-lg flex items-center justify-center gap-2 group/btn transition-opacity ${
