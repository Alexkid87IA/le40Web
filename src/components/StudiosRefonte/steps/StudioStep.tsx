 ...
 * Refonte UX: plus compact, meilleure lisibilité, "Voir plus" pour détails
 */

import { motion } from 'framer-motion';
import { Check, Clock, Star, Eye } from 'lucide-react';
        ...
        {STUDIOS.map((studio) => {
          ...
          return (
            ...
            >
              {/* Recommended badge */}
              {studio.recommended && (
                <div className="absolute -top-2 right-3 px-2 py-0.5 bg-amber-500 rounded-full text-[10px] font-bold text-black flex items-center gap-1 z-10">
                  <Star className="w-2.5 h-2.5" />
                  Top
                </div>
              )}

              {/* Main clickable area - selects studio */}
              <button
              ...
              {/* "Voir plus" link - separate from main click */}
              <div className="px-4 pb-3 -mt-1">
                <button
                  onClick={(e) => {
                    e.stopPropagation();
                    onOpenStudioDetail(studio);
                  }}
                  className="flex items-center gap-1.5 text-xs text-emerald-400 hover:text-emerald-300 transition-colors"
                >
                  <Eye className="w-3.5 h-3.5" />
                  <span className="underline">Voir le détail du studio</span>
                </button>
              </div>
            </div>
          );
