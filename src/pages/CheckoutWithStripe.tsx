...
import {
  Lock, ArrowLeft, Check, AlertCircle, ShoppingCart, Loader2,
...
// =============================================================================
// MAPPING COMPLET DES VARIANT IDS SHOPIFY
// =============================================================================
...
// Salles de réunion
const SALLES_VARIANTS: ProductMapping = {
...
const ESPACES_VARIANTS: ProductMapping = {
...
// Studios créatifs
const STUDIOS_VARIANTS: ProductMapping = {
...
// Domiciliation
const DOMICILIATION_VARIANTS: ProductMapping = {
  'STARTER': {
...
// Équipements
const EQUIPEMENTS_VARIANTS: ProductMapping = {
...
// Tous les mappings combinés
const ALL_VARIANTS: ProductMapping = {
  ...SALLES_VARIANTS,
  ...ESPACES_VARIANTS,
  ...STUDIOS_VARIANTS,
...
export default function CheckoutWithStripe() {
  ...
  const findShopifyVariantId = (item: any): string | null => {
    ...
    if (item.serviceType === 'domiciliation') {
      for (const [planKey, variants] of Object.entries(DOMICILIATION_VARIANTS)) {
        if (serviceName.toUpperCase().includes(planKey)) {
          const durationKey = item.duration === 'year' ? 'annual' : 'monthly';
          return variants[durationKey] || null;
        }
      }
    }

    // 2. Cas général (salles, studios, etc.)
    ...
    if (parts.length >= 2) {
      ...
      for (const [product, variants] of Object.entries(ALL_VARIANTS)) {
        if (product === productName || 
            ...
            productName.includes(product.split(' - ')[0])) {
          
          for (const [variantDuration, variantId] of Object.entries(variants)) {
            if (duration.toLowerCase().includes(variantDuration.toLowerCase()) ||
                variantDuration.toLowerCase().includes(duration.toLowerCase())) {
              return variantId;
            }
          }
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-white font-semibold">Le 40 - 40 Rue de la République, 13002 Marseille</p>
              </div>
            </div>
          </motion.div>
