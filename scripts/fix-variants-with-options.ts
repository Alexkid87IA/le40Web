...
async function updateProduct(productId: number, updates: any) {
  const data = await restRequest(`/products/${productId}.json`, 'PUT', {
    product: updates,
  });
  return data.product;
}
...
async function main() {
  console.log('🔧 AJOUT DES VARIANTS AUX STUDIOS LE 40\n');
  ...
  for (const [handle, variants] of Object.entries(studiosConfig)) {
    ...
    console.log(`  🔧 Configuration des variants avec option "Durée"...`);
    ...
    console.log(`  ✅ Variants créés:`);
    updatedProduct.variants.forEach((v: any) => {
      console.log(`     - ${v.option1}: ${v.price}€`);
    });
